...
class LinkedBST(AbstractCollection):
    ...
    def is_balanced(self):
        ...
        :return:
        """
        vertexes = len([i for i in self])
        return self.height() < 2 * log(vertexes + 1, 2) - 1
    ...
    def rangeFind(self, low, high):
        ...
        if not self.isEmpty():
            ...
            while not stack.isEmpty():
                ...
                if node.right != None and node.right.data <= high:
                    stack.push(node.right)
                if node.left != None and node.left.data >= low:
                    stack.push(node.left)
        return items
        ...
        def add_middle(elements):
            if elements:
                ...
                if len(elements[:index]) >= 1:
                    add_middle(elements[:index])
                if len(elements[index - 1 :]) >= 1:
                    add_middle(elements[index:])
    ...
    def successor(self, item):
        ...
        while probe != None:
            if probe.data >= item:
                if probe.data < current and probe.data != item:
                    current = probe.data
                probe = probe.left
            else:
                probe = probe.right
    ...
    def demo_bst(self, path):
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                tree.add(word)
        # print(tree)
        random_words = random.sample(words_list, 100)
        # print(tree)
        start = time.time()
        ...
        stop = time.time()
        time4 = stop - start
        print("searching 100 words from 1053 in sorted list time:", time1)
        print("searching 100 words from 1053 in unbalanced tree time:", time2)
        print("searching 100 words from unsorted 1053 in unbalanced tree time: ", time4)
        print("searching 100 words from 1053 in balanced tree time: ", time3)
